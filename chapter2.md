...
quantity_data = quantity_data.assign(
    QuantityQuartile = pd.qcut(
        x=quantity_data[_], 
        q=_, 
        labels = range_, _)
    ))
...
quantity_cohorts = quantity_agg.reset_index().pivot(
    index_, 
    columns=_, 
    values=_)
